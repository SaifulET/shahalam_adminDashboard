  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await api.patch('/auth/change-password', {
        userId, // dynamically passed from props or store
        currentPassword,
        newPassword,
      });
    ...
    <div className="min-h-screen py-6 bg-gray-50">
      <div style={{ boxShadow: '0px 1px 6px 0px rgba(0, 0, 0, 0.24)' }} className="mx-auto mt-16 rounded-2xl">
        ...
        <div className="flex items-center px-4 py-4 rounded-tl-2xl rounded-tr-2xl bg-[#71ABE0]">
          <button
            onClick={handleBack}
            className="p-1 mr-3 text-white transition-colors rounded hover:bg-blue-500"
          >
            <ArrowLeft size={20} />
          </button>
        ...
        <div className="max-w-md p-6 mx-auto space-y-5">
          ...
          <Link to="/admin/settings/forget-password" className="underline text-blue-500 text-right">
            Forget Password
          </Link>
...
const PasswordInput = ({ label, value, show, setShow, onChange }) => (
  <div>
